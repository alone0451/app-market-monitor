                ...
                for app in apps:
                    ...
                    try:
                        ...
                        res = collector.collect(
                            ...
                            app_name=app["app_name"], company_name=app["company_name"],
...
def run_b_check(market_id: str, app_id: int, do_download: bool = False):
    ...
    ``do_download`` 只为兼容旧调用保留；DeviceExecutor 会安全拒绝手机端盲搜下载。
                    ...
                    if status == "ok":
                        db.execute(
                            """INSERT INTO results (market_id, app_id, version_name,
                               ...
                               verify_result, screenshot_url, detail, published_at, checked_at)
                               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
                               ON CONFLICT(market_id, app_id) DO UPDATE SET
                               ...
                               screenshot_url=COALESCE(NULLIF(excluded.screenshot_url,''),results.screenshot_url),
                            ...
                            (market_id, app_id,
                             ...
                             str(detail)[:500], normalize_published_at(res.get("published_at")), _now()),
                        )
                    summary["items"].append({
